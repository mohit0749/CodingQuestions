...
func pacificAtlantic(matrix [][]int) [][]int {
	...
	visitedA := make([][]bool, len(matrix))
	visitedP := make([][]bool, len(matrix))
	for i := 0; i < len(matrix); i++ {
		visitedA[i] = append(visitedA[i], make([]bool, len(matrix[i]))...)
		visitedP[i] = append(visitedP[i], make([]bool, len(matrix[i]))...)
	}

	m := len(matrix)
	n := len(matrix[0])
	for i := 0; i < n; i++ {
		dfs(0, i, visitedP, matrix)
		dfs(m-1, i, visitedA, matrix)
	}

	for i := 0; i < m; i++ {
		dfs(i, 0, visitedP, matrix)
		dfs(i, n-1, visitedA, matrix)
	}

	// if we can reach both side
	fmt.Printf("%+v\n%+v\n", visitedA, visitedP)
	ans := make([][]int, 0)
	for i := 0; i < m; i++ {
		for j := 0; j < n; j++ {
...
func dfs(r, c int, visited [][]bool, matrix [][]int) {
	visited[r][c] = true
	if r-1 >= 0 && matrix[r-1][c] >= matrix[r][c] && !visited[r-1][c] {
		dfs(r-1, c, visited, matrix)
	}
	if c-1 >= 0 && matrix[r][c-1] >= matrix[r][c] && !visited[r][c-1] {
		dfs(r, c-1, visited, matrix)
	}
	if r+1 < len(matrix) && matrix[r+1][c] >= matrix[r][c] && !visited[r+1][c] {
		dfs(r+1, c, visited, matrix)
	}
	if c+1 < len(matrix[r]) && matrix[r][c+1] >= matrix[r][c] && !visited[r][c+1] {
		dfs(r, c+1, visited, matrix)
	}
}
